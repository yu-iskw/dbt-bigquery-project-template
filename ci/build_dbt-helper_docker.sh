...
# The script is used to build and push a docker image of dbt-helper.
#
# Usage: build_dbt-helper_docker.sh MODE TARGET DOCKER_TAG
#   - MODE: "build" or "push"
#   - TARGET: "dev" or "prod"
...
# Build docker image
docker build --rm \
  -f "${PROJECT_DIR}"/docker/dbt-helper/Dockerfile \
  -t "$docker_image" \
  "${PROJECT_DIR}"
